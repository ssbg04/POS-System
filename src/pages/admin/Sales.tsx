...
import {
  Loader, Eye, Ban, RotateCcw, AlertCircle,
  Search, X, Printer, MoreHorizontal
} from 'lucide-react';
import { getSales, updateSaleStatus, type Sale } from '../../services/poss/sales';
...
const Sales = () => {
  ...
  const [actionType, setActionType] = useState<'view' | 'void' | 'refund'>('view');
  ...
  const loadData = async () => {
    ...
    try {
      ...
      data.sort((a, b) => new Date(b.sale_date).getTime() - new Date(a.sale_date).getTime());
      setSales(data);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 no-print">
        <div>
          ...
          <p className="text-slate-600 dark:text-slate-400 text-sm">Monitor transactions, refunds, and voided sales.</p>
        </div>
              ...
              {filteredSales.map((sale) => (
                <tr key={sale.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition">
                  <td className="p-4 text-sm text-slate-600 dark:text-slate-300">
                    ...
                    <div className="text-xs text-slate-400">{new Date(sale.sale_date).toLocaleTimeString()}</div>
                  </td>
                  <td className="p-4 font-mono text-xs text-slate-500 dark:text-slate-400">{sale.id?.slice(0, 8)}...</td>
                  <td className="p-4 text-slate-800 dark:text-white font-medium">{sale.customer_name || 'Walk-in'}</td>
                  <td className="p-4 text-slate-800 dark:text-white font-bold">₱{sale.total_amount.toFixed(2)}</td>
                  <td className="p-4 text-sm text-slate-600 dark:text-slate-300">{sale.payment_type}</td>
                  <td className="p-4">
                    <span className={`px-2 py-1 rounded text-xs font-bold uppercase ${getStatusColor(sale.status)}`}>
                      {sale.status}
                    </span>
                  </td>
                  <td className="p-4 text-right relative">
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        // @ts-ignore
                        setActiveDropdown(activeDropdown === sale.id ? null : sale.id);
                      }}
                      className={`p-2 rounded-full transition ${activeDropdown === sale.id ? 'bg-slate-200 dark:bg-slate-700' : 'hover:bg-slate-100 dark:hover:bg-slate-700'}`}
                    >
                          ...
                          <div className="py-1">
                            ...
                            {sale.status === 'completed' && (
                              <>
                                ...
                                >
                                  <RotateCcw size={16} className="text-orange-500" /> Process Refund
                                </button>
                                <button
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            <div className="p-6 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-800/80">
              <h3 className="text-xl font-bold text-slate-800 dark:text-white">
                {actionType === 'view' ? 'Transaction Details' : actionType === 'void' ? 'Void Transaction' : 'Process Refund'}
              </h3>
              <button onClick={() => setShowModal(false)} className="p-1 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-full transition"><X className="text-slate-500 dark:text-slate-400" size={20} /></button>
            </div>
                ...
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm bg-slate-50 dark:bg-slate-900/50 p-4 rounded-lg border border-slate-100 dark:border-slate-700/50">
                    <div>
                      ...
                      <p className="font-mono text-slate-800 dark:text-slate-200 truncate" title={selectedSale.id}>{selectedSale.id}</p>
                    </div>
                    ...
                    <div>
                      <p className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Cashier</p>
                      <p className="font-medium text-slate-800 dark:text-slate-200">{selectedSale.user_name}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Customer</p>
                      <p className="font-medium text-slate-800 dark:text-slate-200">{selectedSale.customer_name || 'Walk-in'}</p>
                    </div>
                  </div>

                  {/* Items Table */}
                  <div>
                    <h4 className="text-sm font-semibold text-slate-800 dark:text-white mb-2">Items Purchased</h4>
                      ...
                      <table className="w-full text-sm text-left">
                        <thead className="bg-slate-50 dark:bg-slate-700/50 text-slate-600 dark:text-slate-300">
                          <tr>
                            ...
                            <th className="p-2 text-right pr-3 font-medium">Total</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                          {selectedSale.items.map((item, idx) => (
                            <tr key={idx}>
                              <td className="p-2 pl-3 text-slate-800 dark:text-slate-200">{item.product_name}</td>
                              <td className="p-2 text-center text-slate-600 dark:text-slate-400">{item.quantity}</td>
                              <td className="p-2 text-right pr-3 text-slate-800 dark:text-slate-200">₱{item.total.toFixed(2)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                  ...
                  <div className="bg-blue-50 dark:bg-blue-900/10 p-4 rounded-lg border border-blue-100 dark:border-blue-900/30 text-sm">
                    <div className="flex justify-between mb-2">
                      ...
                      <span className="font-semibold text-slate-800 dark:text-slate-200 uppercase">{selectedSale.payment_type}</span>
                    </div>
                    <div className="flex justify-between mb-2">
                      <span className="text-slate-600 dark:text-slate-400">Amount Tendered</span>
                      <span className="font-mono text-slate-800 dark:text-slate-200">₱{selectedSale.amount_tendered.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between font-bold border-t border-blue-200 dark:border-blue-800 pt-2 mt-2">
                      <span className="text-slate-700 dark:text-slate-300">Change Due</span>
                      <span className="text-slate-900 dark:text-white">₱{selectedSale.change_due.toFixed(2)}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-center pt-2">
                    <div className="text-center w-full">
                      <div className={`inline-block px-4 py-1.5 rounded-full text-sm font-bold uppercase ${getStatusColor(selectedSale.status)}`}>
                        {selectedSale.status}
                      </div>
                      {selectedSale.status !== 'completed' && (
                        <div className="mt-3 text-xs text-left text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 p-3 rounded-lg border border-red-100 dark:border-red-900/50">
                          <div><strong>Reason:</strong> {selectedSale.refund_reason}</div>
                          <div className="mt-1"><strong>Processed:</strong> {selectedSale.refunded_at ? new Date(selectedSale.refunded_at).toLocaleString() : 'N/A'}</div>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="p-4 bg-red-50 dark:bg-red-900/20 text-red-800 dark:text-red-200 rounded-lg text-sm flex gap-3 items-start border border-red-100 dark:border-red-900/50">
                    ...
                    <div>
                      ...
                      <p className="mt-1 opacity-90">
                        You are about to mark transaction <span className="font-mono">{selectedSale.id}</span> as <strong>{actionType.toUpperCase()}</strong>.
                        This will update reports and cannot be undone.
                      </p>
                    </div>
                  </div>
                  <div>
      ...
      {/* Hidden Print Receipt Template with Barcode and Support QR */}
      {selectedSale && (
        <div id="printable-receipt" className="hidden">
          <div className="text-center mb-4 border-b border-black pb-2">
            <h1 className="text-xl font-bold uppercase">Store POS</h1>
            <p className="uppercase">{selectedSale.status === 'completed' ? 'Official Receipt' : `${selectedSale.status} SLIP`}</p>
            <p className="text-xs mt-1">{new Date(selectedSale.sale_date).toLocaleString()}</p>
            ...
            <p className='text-xs'>Customer: {selectedSale.customer_name}</p>
          </div>
          ...
          <div className="space-y-1 text-xs">
            <div className="flex justify-between">
              <span>Subtotal</span>
              <span>{(selectedSale.total_amount - selectedSale.tax_amount + selectedSale.discount_amount).toFixed(2)}</span>
            </div>
          ...
          {selectedSale.status !== 'completed' && (
            <div className="mt-4 text-center border border-black p-2">
              <p className="font-bold uppercase">*** {selectedSale.status} ***</p>
              <p className="text-xs">Reason: {selectedSale.refund_reason}</p>
              <p className="text-xs">Date: {selectedSale.refunded_at ? new Date(selectedSale.refunded_at).toLocaleString() : ''}</p>
            </div>
          )}
