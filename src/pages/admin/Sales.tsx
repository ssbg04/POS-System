...
const Sales = () => {
  ...
  const submitUpdate = async () => {
    if (!selectedSale || !selectedSale.id) return; // Strict null check
    setProcessing(true);
    try {
      // Explicitly type the status to match Sale interface
      const newStatus: "void" | "refunded" =
        actionType === "void" ? "void" : "refunded";

      await updateSaleStatus(selectedSale.id, newStatus, reason);

      // Update local state to reflect changes immediately without full reload lag
      ...
      // Switch to view mode so user can see/print the result
      setActionType("view");
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Table */}
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow overflow-hidden border border-slate-200 dark:border-slate-700 no-print">
                ...
                >
                  ...
                  <td className="p-4 text-right flex justify-end gap-2">
                    <button
                      ...
                      className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg dark:hover:bg-blue-900/30 transition"
                      title="View Details"
                    >
                      <Eye size={18} />
                    </button>
                    {sale.status === "completed" && (
                      <>
                        <button
                          ...
                          className="p-2 text-orange-600 hover:bg-orange-50 rounded-lg dark:hover:bg-orange-900/30 transition"
                          title="Refund"
                        >
                          <RotateCcw size={18} />
                        </button>
                        <button
                          ...
                          className="p-2 text-red-600 hover:bg-red-50 rounded-lg dark:hover:bg-red-900/30 transition"
                          title="Void"
                        >
                          <Ban size={18} />
                        </button>
